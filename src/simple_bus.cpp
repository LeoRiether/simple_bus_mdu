    ...
    for (int i = 1; i < slave_port.size(); ++i) {
        ...
        for (int j = 0; j < i; ++j) {
            ...
            if (!no_overlap) {
                sb_fprintf(
                    stdout,
                    "Error: overlapping address spaces of 2 slaves : \n");
                sb_fprintf(stdout, "slave %i : %0X..%0X\n", i,
                           slave1->start_address(), slave1->end_address());
                sb_fprintf(stdout, "slave %i : %0X..%0X\n", j,
                           slave2->start_address(), slave2->end_address());
                exit(0);
            }
        }
    }
...
void simple_bus::read(unsigned int unique_priority, int *data,
                      unsigned int address, bool lock) {
    if (m_verbose)
...
void simple_bus::write(unsigned int unique_priority, int *data,
                       unsigned int address, bool lock) {
    if (m_verbose)
...
simple_bus_status simple_bus::burst_read(unsigned int unique_priority,
                                         int *data, unsigned int start_address,
                                         unsigned int length, bool lock) {
    if (m_verbose) {
...
void simple_bus::handle_request() {
    ...
    if (m_current_request->do_write)
        slave_status =
            slave->write(m_current_request->data, m_current_request->address);
    else
        slave_status =
            slave->read(m_current_request->data, m_current_request->address);
    ...
    switch (slave_status) {
        ...
        case SIMPLE_BUS_OK:
            ...
            if (m_current_request->address > m_current_request->end_address) {
                ...
                m_current_request->transfer_done.notify();
                m_current_request = (simple_bus_request *)0;
            } else {  // more data to transfer, but the (atomic) slave transfer
                      // is done
                m_current_request = (simple_bus_request *)0;
            }
            break;
        case SIMPLE_BUS_WAIT:
            // the slave is still processing: no clearance of the current
            // request
            break;
        default:
            break;
